...
export const PatientDashboard: React.FC = () => {
  ...
  const upcomingAppointments = getUpcoming(patientId);
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Link
          ...
          className="bg-white p-5 rounded-2xl border border-slate-200 shadow-soft hover:border-[#0D7A5F]/40 transition-all flex items-center gap-4 group"
        >
          <div className="w-12 h-12 rounded-xl bg-emerald-50 text-[#0D7A5F] flex items-center justify-center shrink-0 group-hover:scale-105 transition-transform">
      ...
      {/* 3. Upcoming Appointment Card (Live Data from Store) */}
      <div className="bg-white rounded-3xl border border-slate-200 shadow-soft p-6 space-y-4">
        <div className="flex items-center justify-between pb-3 border-b border-slate-100">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-bold text-slate-900">Next Scheduled Consultation</h2>
          </div>
          {nextAppointment && (
        ...
        {nextAppointment ? (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-center">
            ...
            <div className="flex items-start gap-4">
              <img
                src={nextAppointment.doctorAvatarUrl}
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-slate-500 flex items-center gap-1">
                  ...
                  {nextAppointment.clinicName}, {nextAppointment.city}
                </p>
              </div>
            </div>
            ...
            <div className="space-y-1.5 text-xs text-slate-600 bg-slate-50 p-3.5 rounded-2xl border border-slate-100">
              <div className="flex items-center justify-between">
                ...
                <span className="font-bold text-slate-800">
                  {appointmentService.formatAppointmentDate(nextAppointment.date)}
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-slate-400">Time (PKT):</span>
                <span className="font-bold text-slate-800">{nextAppointment.time}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-slate-400">Type:</span>
                <span className="font-semibold text-[#0D7A5F]">
                  {nextAppointment.consultationType === 'online' ? 'Video Consultation' : 'In-Clinic Visit'}
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-slate-400">Fee:</span>
                <span className="font-bold text-slate-900">
                  {formatPKR(nextAppointment.consultationFee)}
                </span>
              </div>
            </div>
          ...
          <div className="text-center py-6 space-y-3">
            ...
            <div className="text-sm font-bold text-slate-800">No Upcoming Appointments</div>
            <p className="text-xs text-slate-500 max-w-sm mx-auto">
              You do not have any pending or scheduled consultations. Book a slot with a verified doctor in Karachi or Lahore.
            </p>
            <Link to="/find-doctors">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          ...
          <Link
            ...
            className="p-5 rounded-2xl bg-white border border-slate-200 shadow-soft hover:border-[#0D7A5F]/40 hover:shadow-card-hover transition-all text-center group"
          >
            <div className="w-10 h-10 rounded-xl bg-emerald-50 text-[#0D7A5F] flex items-center justify-center mx-auto mb-3 group-hover:scale-110 transition-transform">
              <Calendar className="w-5 h-5" />
            </div>
            <div className="text-xs font-bold text-slate-900 group-hover:text-[#0D7A5F]">
              My Appointments
            </div>
            <div className="text-[11px] text-slate-400 mt-0.5">{upcomingAppointments.length} scheduled</div>
          </Link>
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          ...
          <Link to="/find-doctors" className="text-xs font-semibold text-[#0D7A5F] hover:underline flex items-center gap-1">
